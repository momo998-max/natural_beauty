...
class VerifyUserController extends AbstractController
{
    ...
    public function verifyUuser(Request $request, UserRepository $repo, EntityManagerInterface $em): Response
    {
        ...
        if (!isset($data["tel"])) {
            return $this->json([
                'message' => 'Numero de telephone est aubligatoire',
            ], 204);    
        }
        ...
        return $this->json([
            'message' => 'Numero de telephone valide veuiller renvoiyer le code recus par sms',
        ], 200);
    }
     ...
     */
    public function verifyCecretCode(Request $request, CodeRepository $rpcode, EntityManagerInterface $em): Response
    {
        $data = \json_decode($request->getContent(), true);
        if (!isset($data["code"])) {
            return $this->json([
                'message' => 'Le code d\activation est aubligatoire',
            ], 204);    
        }
